...
func ProcHeartBeatReq(session *session.Session, reader *packet.RawPacket) []byte {
...
func ProcGetSeedReq(session *session.Session, reader *packet.RawPacket) []byte {
...
func ProcUserLoginReq(session *session.Session, reader *packet.RawPacket) []byte {
	...
	if prevSession, ok := p.(*session.Session); ok {
	...
	fetcherTask := func(session *session.Session) {
		for {
...
func ProcOfflineReq(session *session.Session, reader *packet.RawPacket) []byte {
	session.SetFlagKicked()
	return nil
}
